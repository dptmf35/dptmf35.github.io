---
title: "[공부] 로봇 캘리브레이션을 위한 Kabsch 알고리즘 — 기준점·측정점 정렬의 수학"
excerpt: "로봇 base·tool 캘리브레이션에서 기준점과 측정점을 잇는 최적 rigid transform (R, t)을 닫힌형으로 구하는 Kabsch 알고리즘을, 문제 정의부터 SVD 유도·reflection 처리·구현·잔차 해석까지 정리한다."
...
> 로봇 base와 tool을 캘리브레이션할 때, 우리는 **알고 있는 기준점**과 **센서로 잰 측정점**이라는 두 벌의 대응하는 점을 손에 쥔다.
...
로봇 캘리브레이션의 뼈대는 같다. 좌표계 A에서 알고 있는 점들과, 좌표계 B에서 그 점들을 잰 측정값이 **쌍(pair)** 으로 주어진다.

- **base 캘리브레이션**: 외부 tracker/camera가 잰 점 ↔ 로봇 base 좌표계에서 아는 점
- **tool(TCP) 캘리브레이션**: 여러 자세에서 잰 tool 끝점 ↔ 명령한 목표점
...
$$d = +1$$ 이면 그대로, $$d = -1$$ 이면 가장 작은 특이값 방향만 뒤집어 반사를 회전으로 바로잡는다. 이때 잃는 정확도는 **가장 작은 $$\sigma$$ 에 해당하는 축**에 국한되므로 손해가 가장 적다. 이 한 줄을 빠뜨리면 드물게 좌우가 뒤집힌 캘리브레이션이 나온다.
...
## 7. 잔차와 outlier — 캘리브레이션 품질 판단

정렬 후 남는 **RMSD가 캘리브레이션 오차의 지표**다.
...
- 정렬 후 **RMSD가 캘리브레이션 오차**이며, outlier에는 RANSAC·robust weighting을 얹는다
