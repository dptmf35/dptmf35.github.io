...
- BOP 벤치마크에서 SOTA급 성능, **추적**까지 한 모델로 처리

> 입력은 보통 **RGB-D + 물체 세그멘테이션 마스크 + (CAD 또는 참조영상)** 이다. 첫 프레임은 추정, 이후는 추적으로 이어갈 수 있어 매니퓰레이션 루프에 적합하다.
...
- 각 마스크를 대상 물체의 **템플릿(렌더링)** 과 의미(semantic)·외형·기하 관점에서 매칭해, 찾는 물체 인스턴스를 골라냄
...
![FoundationPose는 render-and-compare, SAM-6D는 SAM 세그멘테이션 + 점 매칭](/assets/images/6dof-foundationpose-vs-sam6d.svg)
...
- 다만 **세그멘테이션 마스크**와 (대개) **CAD 또는 참조영상**, 그리고 상당한 **연산 자원**이 필요하다.
...
| 상황 | 추천 접근 |
| --- | --- |
| 캘리브레이션·고정 지그·마커 부착 가능 | [마커 & 기하 기반]({% post_url 2026-07-02-6dof-marker-geometry-based %}) |
