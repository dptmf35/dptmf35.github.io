...
- 카메라 좌표계의 포즈를 **로봇 베이스 좌표계**로 옮기려면 **hand-eye calibration**(카메라↔엔드이펙터 변환)이 필요하다. 이 캘리브레이션 자체에도 마커 기반 포즈 추정이 쓰인다.
...
→ [포인트 클라우드 정합 기반 6DoF 추정]({% post_url 2026-07-03-6dof-point-cloud-registration %})
...
- **환경을 통제할 수 있으면** 마커 기반이 가장 실용적이다(특히 캘리브레이션).
