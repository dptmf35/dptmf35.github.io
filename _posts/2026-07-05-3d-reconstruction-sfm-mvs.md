...
새 카메라가 등록되면, 그 이미지에서 처음 보이거나 이제야 두 시점 이상을 확보한 track들을 **triangulation**해 3D 점을 늘린다. 그리고 다음 이미지로 넘어가 **등록 → triangulation → (주기적) bundle adjustment**를 모든 이미지가 붙을 때까지 반복한다.
