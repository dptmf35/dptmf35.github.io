...
$$E$$ 의 자유도는 **5**다 — 회전 3 + 평행이동 방향 2. $$t$$ 의 **크기는 알 수 없다**(scale ambiguity): 장면 전체를 2배 키우고 카메라를 2배 멀리 놓아도 같은 이미지가 나온다.
...
- $$F e = 0$$: epipole $$e$$ 는 $$F$$ 의 null vector — 그래서 $$F$$ 는 **rank 2**
...
- 예: $$w = 0.5$$ 일 때 $$s = 8$$ 이면 $$N \approx 1177$$, $$s = 5$$ 면 $$N \approx 145$$. **표본이 작을수록 기하급수적으로 유리** — five-point algorithm이 선호되는 실질적 이유
